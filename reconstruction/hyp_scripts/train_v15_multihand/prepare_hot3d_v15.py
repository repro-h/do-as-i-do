...
def main():
    ...
    # HOT3D imports its Quest provider eagerly, even for Aria sequences. Avoid
    # importing Quest-only pyvrs2/vrsbindings dependencies in this Aria export.
    import types

    class _UnavailableQuestProvider:
        def __init__(self, *args, **kwargs):
            raise RuntimeError(
                "Quest dependencies are unavailable; this exporter supports "
                "HOT3D Aria recording.vrs sequences"
            )

    quest_stub = types.ModuleType("data_loaders.QuestDataProvider")
    quest_stub.QuestDataProvider = _UnavailableQuestProvider
    sys.modules.setdefault("data_loaders.QuestDataProvider", quest_stub)
