#!/usr/bin/env python3
"""Refine HandFlow ray depth from local Pi3X points and decoder features."""
...
MODEL_VERSION = "v11_1_pi3x_metric_point_ray_residual_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--feature-mode",
        choices=(
            "normal", "decoder_zero", "point_zero", "metric_zero",
            "all_pi3x_zero", "spatial_shuffle", "time_reverse",
        ),
        default="normal",
    )
...
class Pi3XMetricPointRayResidual(nn.Module):
    def __init__(
        self,
        decoder_dim: int,
        metric_dim: int,
        ...
        self.decoder_encoder = nn.Sequential(
            ...
            nn.Linear(decoder_dim, args.token_dim),
        )
        # XYZ and its norm expose Pi3X geometry without using HandFlow depth.
        self.point_encoder = nn.Sequential(
            nn.LayerNorm(4),
            nn.Linear(4, args.token_dim),
            nn.GELU(),
        ...
        self.frame_encoder = nn.Sequential(
            ...
            nn.Linear(args.token_dim * 3, args.hidden_dim),
            nn.GELU(),
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if mode in ("decoder_zero", "all_pi3x_zero"):
            decoder = torch.zeros_like(decoder)
        if mode in ("point_zero", "all_pi3x_zero"):
            points = torch.zeros_like(points)
        if mode in ("metric_zero", "all_pi3x_zero"):
            metric = torch.zeros_like(metric)
        if mode == "spatial_shuffle":
            decoder = torch.roll(decoder, shifts=7, dims=3)
            points = torch.roll(points, shifts=7, dims=3)
        ...
        token = (
            ...
            + self.metadata_encoder(batch["neighborhood_metadata"])
        )
        ...
        temporal, _ = self.temporal(
            self.frame_encoder(torch.cat((wrist, pooled, metric_token), dim=-1))
        )
...
def main() -> None:
    ...
    model = Pi3XMetricPointRayResidual(
