#!/usr/bin/env python3
"""Train a hand-local SE(3) residual from localized Pi3X geometry."""
...
MODEL_VERSION = "object_frame_hand_local_pi3x_se3_residual_v2"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-normalized-residual", type=float, default=3.0)
    ...
    parser.add_argument("--w-translation-residual", type=float, default=0.002)
    parser.add_argument("--w-rotation-residual", type=float, default=0.002)
...
def localize_token_metadata(
    hand_metadata: torch.Tensor,
    hand_valid: torch.Tensor,
    key_metadata: torch.Tensor,
    initial_rotation: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Express Pi3X xyz around the observed hand in the initial hand frame."""
    confidence = hand_metadata[..., 6].clamp(0.0, 1.0)
    weight = confidence * hand_valid.to(confidence.dtype)
    fallback = hand_valid.to(confidence.dtype)
    weight = torch.where(
        weight.sum(-1, keepdim=True) <= 1e-6, fallback, weight
    )
    center = (
        hand_metadata[..., :3] * weight[..., None]
    ).sum(dim=2) / weight.sum(dim=2, keepdim=True).clamp_min(1.0)

    def convert(metadata: torch.Tensor) -> torch.Tensor:
        output = metadata.clone()
        centered = metadata[..., :3] - center[:, :, None, :]
        output[..., :3] = torch.einsum(
            "btnc,btcd->btnd", centered, initial_rotation
        )
        return output

    return convert(hand_metadata), convert(key_metadata)


class HandLocalGeometryResidualModel(nn.Module):
    def __init__(
        self,
        local_hand_dim: int,
        ...
        self.max_normalized_residual = max_normalized_residual
    ...
    def forward(
        ...
        delta_rotation = rotation_6d_to_matrix(self.rotation_head(temporal))
...
def run_epoch(
    ...
    values = {
        name: []
        for name in ("initial_t", "predicted_t", "initial_r", "predicted_r")
    }
    batches = 0
    iterator = tqdm(loader, desc="train" if training else "val")
    for batch in iterator:
        batch = {key: value.to(device) for key, value in batch.items()}
        initial_r = batch["initial_rotation"]
        local_hand_metadata, local_key_metadata = localize_token_metadata(
        ...
        with torch.set_grad_enabled(training):
            predicted_delta_t, predicted_delta_r = model(
            ...
            target_delta_object = (
                batch["target_translation"] - batch["initial_translation"]
            )
            target_delta_local = torch.einsum(
                "btc,btcd->btd", target_delta_object, initial_r
            )
            predicted_delta_object = torch.einsum(
                "btc,btdc->btd", predicted_delta_t, initial_r
            )
            predicted_t_normalized = (
                batch["initial_translation"] + predicted_delta_object
            )
            predicted_r = initial_r @ predicted_delta_r
            ...
            translation_frame = smooth_l1_vector(
                predicted_delta_t, target_delta_local, beta
            )
            ...
            initial_r_error = rotation_angle(
                initial_r, batch["target_rotation"]
            )
            rotation_weight = (
                batch["valid_rotation"].to(predicted_r_error.dtype)
                * batch["rotation_weight"]
            )
            rotation = masked_mean(predicted_r_error, rotation_weight)
            ...
            translation_residual = masked_mean(
                torch.linalg.norm(predicted_delta_t, dim=-1),
                batch["valid_translation"],
            )
            identity = torch.eye(3, device=device, dtype=predicted_delta_r.dtype)
            rotation_residual = masked_mean(
                rotation_angle(predicted_delta_r, identity), rotation_weight
            )
            total = (
                ...
                + args.w_translation_residual * translation_residual
                + args.w_rotation_residual * rotation_residual
            )
            if training:
        ...
        metric_rows = (
            ("initial_t", torch.linalg.norm(initial_t - target_t, dim=-1), valid_t),
            ("predicted_t", torch.linalg.norm(predicted_t - target_t, dim=-1), valid_t),
            ("initial_r", initial_r_error, valid_r),
            ("predicted_r", predicted_r_error, valid_r),
        )
        for name, metric, valid in metric_rows:
            values[name].append(metric.detach().cpu().numpy()[valid])
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        checkpoint = {
            ...
            "initial_pose_usage": "token_localization_and_output_composition_only",
        }
