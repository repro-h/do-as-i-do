...
PORT=${PORT:-8098}
TRANSFORM_MODE=${TRANSFORM_MODE:-translation_only}
...
PREDICTION_ROOT=$GLOBAL_V2_ROOT/predictions/pi3x_ray_depth_only_full_v1
...
GT_OBJECT_JSON=$GLOBAL_V2_ROOT/optim_seq/sam_to_ycb_pose_consensus_pilot_v3/$STREAM_ID/dexycb_gt_pose_in_sam_canonical.json
VIS_ROOT=$GLOBAL_V2_ROOT/visualization/pi3x_ray_depth_only_full_v1_val
STREAM_VIS=$VIS_ROOT/$STREAM_ID
OUT_DIR=$GLOBAL_V2_ROOT/optim_seq/relative_hand_target_pilot_v1/$STREAM_ID
...
IFS=$'\t' read -r HAND_SIDE OBJECT_MESH OBJECT_SCALE < <(
  "$PI3_PYTHON" -c '
import json, sys
