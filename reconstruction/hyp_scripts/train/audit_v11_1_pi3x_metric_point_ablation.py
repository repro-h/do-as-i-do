#!/usr/bin/env python3
"""Audit V11.1 ray-residual dependence on Pi3X input branches."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--modes",
        default=(
            "normal,decoder_zero,point_zero,metric_zero,all_pi3x_zero,"
...
def summarize(rows: list[dict]) -> dict:
    ...
    residual = np.asarray([row["residual"] for row in rows])
    ray = initial / np.maximum(
        np.linalg.norm(initial, axis=-1, keepdims=True), 1e-6
    )
    target_residual = np.sum((target - initial) * ray, axis=-1)
    corrected = initial + residual[:, None] * ray
    initial_full = np.linalg.norm(initial - target, axis=-1)
    corrected_full = np.linalg.norm(corrected - target, axis=-1)
    increase = corrected_full - initial_full
    return {
        "initial_ray_depth": distribution(np.abs(target_residual)),
        "corrected_ray_depth": distribution(
            np.abs(residual - target_residual)
        ),
        ...
        for batch in loader:
            ...
            for batch_index, time_index in zip(*torch.where(valid)):
                ...
                row["residuals"].append(
                    float(residual[batch_index, time_index])
                )
                row["initial"] = batch["initial_t"][
                    batch_index, time_index
                ].cpu().numpy()
                row["target"] = batch["target_t"][
                    batch_index, time_index
                ].cpu().numpy()
                row["side"] = int(batch["side"][batch_index, time_index])
    rows = [{
        **row,
        "residual": float(np.mean(row["residuals"])),
