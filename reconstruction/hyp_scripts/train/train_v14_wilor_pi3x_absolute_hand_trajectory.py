#!/usr/bin/env python3
"""Train an absolute camera-frame hand trajectory from WiLoR and Pi3X.

WiLoR contributes only canonical-right 2D joint locations used as
cross-attention queries. No WiLoR weak-perspective depth or camera translation
is used. The model predicts absolute depth and a 2D wrist offset, which are
composed through the camera intrinsics into an XYZ trajectory.
"""
...
MODEL_VERSION = "v14_wilor_pi3x_absolute_hand_trajectory_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--query-train-root", required=True)
    parser.add_argument("--query-val-root", required=True)
...
def query_path(root: Path, stream_id: str) -> Path:
    return (root / stream_id / "wilor_query_cache.npz").resolve()


def normalized_frame_ids(values: np.ndarray) -> list[str]:
    return [str(value).rsplit("_", 1)[-1].zfill(6) for value in values]
...
class DenseTrajectoryDataset(Dataset):
    """Expose Pi3X grids and WiLoR-derived canonical-right 2D queries."""
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        query_file = query_path(self.query_root, stream_id)
        query = load_npz(str(query_file))
        if scalar_text(query["stream_id"]) != stream_id:
            raise ValueError(f"WiLoR stream mismatch: {query_file}")
        query_frames = normalized_frame_ids(query["frame_ids"])
        global_frames = normalized_frame_ids(glob["frame_ids"])
        if query_frames != global_frames:
            raise ValueError(f"WiLoR/global frame mismatch: {query_file}")
        query_mirrored = bool(np.asarray(
            query["canonical_right_horizontal_mirror"]
        ).item())
        if query_mirrored != cache_mirrored:
            raise ValueError(
                f"WiLoR/Pi3X canonical-right mismatch: {query_file}"
            )
        ...
        uv01 = np.asarray(
            query["joints_uv01_canonical_right"], dtype=np.float32
        )[start:end]
        pixels = uv01 * np.maximum(image_wh - 1.0, 1.0)[None, None]
        query_valid = np.asarray(
            query["joint_query_valid"], dtype=bool
        )[start:end].copy()
        query_confidence = np.asarray(
            query["joint_query_confidence"], dtype=np.float32
        )[start:end].copy()
        query_valid &= np.isfinite(pixels).all(axis=-1)
        ...
        observed = np.asarray(query["model_valid"], dtype=bool)[start:end]
        ...
        return {
            ...
            "joint_query_confidence": torch.from_numpy(
                finite_float(query_confidence)
            ),
...
class Pi3XAbsoluteTrajectoryModel(nn.Module):
    ...
    def forward(
        ...
        query_metadata = torch.cat((
            ...
            batch["joint_query_confidence"][..., None],
        ), dim=-1)
...
def checkpoint_payload(
    ...
    return {
        ...
        "query_source": "wilor_canonical_right_2d_joints",
        "initial_pose_usage": "wilor_2d_query_and_output_ray_only",
        "output_parameterization": "absolute_depth_plus_normalized_uv_offset",
        "wilor_weak_perspective_depth_input": False,
...
def main() -> None:
    ...
    train_data = DenseTrajectoryDataset(
        ...
        Path(args.dense_train_root), Path(args.query_train_root),
        args.query_dropout,
    )
    val_data = DenseTrajectoryDataset(
        Path(args.val_windows), Path(args.global_val_root),
        Path(args.dense_val_root), Path(args.query_val_root), 0.0,
    )
